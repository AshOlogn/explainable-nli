...
class ESNLIExplanationDataset(Dataset):
...
class ANLIExplanationDataset(Dataset):
    def __init__(self, level, split, device):
        data = json.load(open(f'data/anli/{level}/{split}.json'))
        self.tokenizer = BertTokenizer.from_pretrained('prajjwal1/bert-tiny')
        self.X = [example['explanation'] for example in data if len(example['explanation']) > 0]
        self.y = [example['label'] for example in data if len(example['explanation']) > 0]
        self.device = device
